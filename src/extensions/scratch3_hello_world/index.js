...
class Scratch3HelloBlocks {
     ...
     */
     _getState (target) {
        let state = target.getCustomState(Scratch3HelloBlocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(Scratch3HelloBlocks.DEFAULT_HELLOWORLD_STATE);
            target.setCustomState(Scratch3HelloBlocks.STATE_KEY, state);
        }
        return state;
    }
